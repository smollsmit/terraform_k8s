...
resource "google_compute_network" "vpc" {                                                                                                                                                                   
  name                    =  "${format("%s","${var.organization}-${var.project_name}-${var.env}-vpc")}"
...
resource "google_compute_firewall" "allow_web_from_all" {
    name    = "${var.project_name}-allow-web-from-all"
    network = "${google_compute_network.vpc.name}"
    allow {
        protocol = "tcp"
        ports    = ["80","443"]
...
resource "google_compute_firewall" "allow_ssh_from_all" {
    name    = "${var.project_name}-allow-ssh-from-all"
    network = "${google_compute_network.vpc.name}"
    allow {
        protocol = "tcp"
        ports    = ["22"]
...
resource "google_compute_firewall" "allow_ssh_from_trusted" {
    name    = "${var.project_name}-allow-ssh-from-trusted"
    network = "${google_compute_network.vpc.name}"
    allow {
        protocol = "tcp"
        ports    = ["22"]
...
resource "google_compute_firewall" "allow_all_from_trusted" {
  name    = "${var.project_name}-allow-all-from-trusted"
  network = "${google_compute_network.vpc.name}"
